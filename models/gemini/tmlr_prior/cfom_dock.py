...
from models.gemini.tmlr_prior.cvae import ConditionalPriorCVAE
...
class CfomDockGemini(CfomDock):
    ...
    def __init__(self,src_smiles_encoder, use_cvae=True, *args,**kws):
        
        super().__init__(*args,**kws)    
        self.src_smiles_encoder = src_smiles_encoder
        ...
        if use_cvae:
            self.cvae = ConditionalPriorCVAE(
                d_model=self.graph_encoder.out_channels,
                d_latent=32,
            )
    
    def _create_src_smiles_memory(self, src_smiles_tokens, graph_data):
        smiles_padding_mask = self.src_smiles_encoder.create_src_key_padding_mask(
            src_smiles_tokens
        )
        smiles_memory = self.src_smiles_encoder(src_smiles_tokens, smiles_padding_mask)
        if (
        ...
        num_repeats = graph_data['ligand'].num_frags
        
        return smiles_memory.repeat_interleave(num_repeats, dim=0), smiles_padding_mask.repeat_interleave(num_repeats, dim=0)
    ...
    def optimized_generate_samples(
        ...
        **kwargs
        ...
        smiles_memory, smiles_padding_mask = self._create_src_smiles_memory(graph_data['ligand'].mol_tokens, graph_data)

        src_smiles_embedding = self.masked_mean_pooling(
        ...
        for i in range(combined_memory.shape[0]):
            ...
            while len(chains) < num_samples:
                ...
                curr_src_smiles_embedding = src_smiles_embedding[i].repeat(num_samples, 1)
                if self.use_cvae:
                    c_bos, kld_loss = self.cvae(
                        c_bos,curr_src_smiles_embedding, None, training=False
                    )
                else:
    ...
    def forward(
        ...
        if self.use_cvae:
            frag_smiles_memory, frag_smiles_padding_mask = self._create_text_memory(smiles_tokens_tgt)
            full_smiles_memory, full_smiles_padding_mask = self._create_src_smiles_memory(graph_data['ligand'].mol_tokens, graph_data)
            fragment_embedding = self.masked_mean_pooling(
                frag_smiles_memory, ~frag_smiles_padding_mask        
            )
            src_smiles_embedding = self.masked_mean_pooling(
                full_smiles_memory, ~full_smiles_padding_mask        
            )
            graph_tgt_start, kld_loss = self.cvae(
                graph_tgt_start, src_smiles_embedding, fragment_embedding, training=True
            )
        else:
